...
# ========== 配置参数 ==========
class Config:
    # 模型设置，teacher_model应该是一个比较强的模型，例如"Qwen/Qwen-72B"，作为演示可以跑个7B的（可能看不出效果）    # student_model是一个小的模型
    # 这里以Qwen模型为例，实际使用时请替换为合适的模型名称
    teacher_model_name = "Qwen/Qwen-7B"
    student_model_name = "Qwen/Qwen-1.8B"

    # 训练参数，要根据设备性能和数据集大小进行调整
    batch_size = 16
    num_epochs = 3
    learning_rate = 2e-5
    max_seq_length = 512
...
class DistillationLoss:
    ...
    def calculate(
        ...
        #kl_loss-相对熵，用于衡量两个模型损失分布之间的差异，将这个损失传给学生模型促使其向教师模型靠拢
        kl_loss = F.kl_div(
            soft_student,
            soft_teacher,
            reduction="batchmean",
