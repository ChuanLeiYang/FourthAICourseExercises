...
# 定义一个函数，用于将文本数据转换为模型所需的格式，即文本编码
def collate_fn(data):
    ...
    # 复制输入ID作为标签，用于语言模型训练，生成模型的输入也是标签
...
def train():
    ...
    #判断"params"文件夹是否存在，不存在则创建，用于保存模型参数
    import os
    ...
    for epoch in range(EPOCH):  # 循环每一轮训练
        for i, data in enumerate(loader):  # 遍历数据加载器中的批次
            ...
            if i % 50 == 0:  # 每隔50个批次打印一次信息
                ...
                # 打印训练信息，生成模型loss只在刚开始的时候看下是否呈下降趋势，后面就没有参考意义了，主要看acc
